...
def total_cost_shopping():
    total_cost = 0
    for key in price_list.keys(): # loop through price list
        if key in quantity_list: # check if fruit is in quantity list
            total_cost += price_list[key]*quantity_list[key]
    
    print("total cost = ", total_cost)
    return total_cost # must return total cost for testing purpose
...
def cost_of_fruits(fruit, quantity):
    ...
    return cost # must return total cost for testing purpose
